...
export const getDetailCourses = createAsyncThunk(
  ...
  async (params, thunkAPI) => {
    let {id,page,limit=1}=params
    const response = await axios.get(`${BASE_URL_API}/courses/${id}?page=${page}&limit=${limit}`);
...
export const postLession = createAsyncThunk(
  ...
  async (params, thunkAPI) => {
    const response = await axios.post(`${BASE_URL_API}/courses/lessions`,params);
    console.log('response lession',response)
    if (response.status === 201) {
      return response.data.data;
    }
  }
...
export const deleteLession = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    const response = await axios.delete(`${BASE_URL_API}/courses/lessions/${id}`);
    if (response.status === 200) {
      return id;
    }
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getDetailCourses.fulfilled, (state, action) => {
        ...
        state.lessons.perPage = action.payload.perPage;
      ...
      .addCase(postLession.fulfilled, (state, action) => {
        ...
        state.lessons.totalPages = Math.ceil((state.lessons.totalCount) / state.lessons.list.length)
      })
      .addCase(deleteLession.fulfilled, (state, action) => {
        state.lessons.list = state.lessons.list.filter((item) => item.id !== action.payload);
        state.lessons.totalCount -= 1;
        state.lessons.totalPages = Math.ceil((state.lessons.totalCount) /  state.lessons.perPage)
      })
